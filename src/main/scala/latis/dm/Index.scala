...
object Index {
  ...
  def apply(): Index = new AbstractVariable(metadata = Metadata("index")) with Index
  
  def withLength(length: Int): Index = new AbstractVariable(metadata = Metadata("index"), data = IndexData(length)) with Index

  def apply(value: Int): Index = new AbstractVariable(metadata = Metadata("index"), data = IndexValue(value)) with Index
