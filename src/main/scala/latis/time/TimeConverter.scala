...
object TimeConverter {
  ...
  def apply(scale1: TimeScale, scale2: TimeScale): TimeConverter = {
    if (scale1 == scale2) NoOpTimeConverter(scale1, scale2)
      ...
      case (TimeScaleType.TAI, TimeScaleType.UTC) => new TaiToUtcConverter(scale1, scale2)
      case (TimeScaleType.UTC, TimeScaleType.TAI) => new UtcToTaiConverter(scale1, scale2)
      case (TimeScaleType.TAI, TimeScaleType.TAI) if (scale1.epoch != scale2.epoch) => new TaiToTaiConverter(scale1, scale2)
