...
class IndexSet(val start: Int, val stop: Int, val stride: Int) extends DomainSet {
  ...
  override def indexOf(data: Data): Int = data match {
  ...
  override def hashCode: Int = start + 7 * stop + 17 * stride
...
object IndexSet {
  ...
  def apply(): IndexSet = new IndexSet(0, Int.MaxValue-1, 1) //TODO: support infinite? length = -1?
