...
class MinFilter(name: String) extends Filter {
  ...
  var currentMin = Scalar("99999999999999999999.0") //Wrong. This should probably be first Scalar of name "name"
			...
			scalar match {
			  ...
			  case s: Scalar => if (scalar.hasName(name)) { 
				  val comparison = s.compare(currentMin)
						if (comparison < 0) {
						  //Found a new min value, so initiate grand master plan...
						  keepSamples.clear 
						  currentMin = s
						  Some(scalar) 
					  }
