...
object StrideFilter extends OperationFactory {
  ...
  override def apply(args: Seq[String]): StrideFilter = {
    if (args.length > 1) throw new LatisServiceException("The StrideFilter accepts only one argument")
    try {
      StrideFilter(args.head.toInt)
    } catch {
      case e: NumberFormatException => throw new LatisServiceException("The StrideFilter requires an integer argument")
    }
  }
