...
class TestAddition {
  ...
  @Test
  def test_real = {
    assertEquals(Dataset(Real(6.28), TestDataset.real.getMetadata), TestDataset.real + 3.14)
    assertEquals(Dataset(Real(6.28)), 3.14 + TestDataset.real)

  }
  @Test
  def test_integer = {
    assertEquals(Dataset(Real(40), TestDataset.integer.getMetadata), TestDataset.integer + (-2))
    assertEquals(Dataset(Real(40)), (-2) + TestDataset.integer)
  }
  //@Test
  def test_text = {
  ...
  @Test
  def test_time = {
    assertEquals(Dataset(Real(1500.0), TestDataset.real_time.getMetadata), TestDataset.real_time + 500)
    assertEquals(Dataset(Real(1500.0)), 500 + TestDataset.real_time)
  }
  //@Test
  def test_binary = {
  ...
  def test_tuple_of_tuples = {
    assertEquals(Dataset(Tuple(Tuple(Real(10), Real(10)), Tuple(Real(11), Real(11.1))), TestDataset.tuple_of_tuples.getMetadata), TestDataset.tuple_of_tuples + 10)
    assertEquals(Dataset(Tuple(Tuple(Real(10), Real(10)), Tuple(Real(11), Real(11.1)))), 10 + TestDataset.tuple_of_tuples)
  }
  //@Test
  ...
  def test_function_of_scalar = {
    val exp1 = Dataset(Function(List(Sample(Real(0), Real(1)), 
                       Sample(Real(1), Real(2)), 
                       Sample(Real(2), Real(3)))), TestDataset.function_of_scalar.getMetadata)
    val exp2 = Dataset(Function(List(Sample(Real(0), Real(1)), 
                       Sample(Real(1), Real(2)), 
                       Sample(Real(2), Real(3)))))
  ...
  def test_index_function = {
    val exp1 = Dataset(Function(List(Integer(0), Integer(1))), TestDataset.index_function.getMetadata)
    val exp2 = Dataset(Function(List(Integer(0), Integer(1))))
    assertEquals(exp1, TestDataset.index_function + (-1))
    assertEquals(exp2, (-1) + TestDataset.index_function)
  }
