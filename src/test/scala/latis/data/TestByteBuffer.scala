...
class TestByteBuffer {
  ...
  //TODO: re-enable Iterable Data tests
...
//  for (data <- datas; test <- tests) 
  ...
  val doubleDatum = Data(ByteBuffer.allocate(8).putDouble(3.14).rewind.asInstanceOf[ByteBuffer])
  ...
  val doubleDatumRecords = Data(ByteBuffer.allocate(24).putDouble(1.0).putDouble(2.0).putDouble(3.0))
...
//  val doubleDataRecords = Data(ByteBuffer.allocate(32).putDouble(1.0).putDouble(2.0).putDouble(3.0).putDouble(4.0), 16)
  val doubleDataRecords = Data(ByteBuffer.allocate(32).putDouble(1.0).putDouble(2.0).putDouble(3.0).putDouble(4.0))
  ...
  val stringDataRecords = Data("HelloWorld".foldLeft(ByteBuffer.allocate(20))(_.putChar(_)))
