...
class TestNestedFunction {
  ...
  def select_inner_domain {
    ...
    val f = v.asInstanceOf[Function].iterator.next.asInstanceOf[Sample].range.asInstanceOf[Function]
    val n = f.getLength
    assertEquals(3, n)
  }
  ...
  def select_inner_range {
    ...
    val dataMaps = v.asInstanceOf[Function].iterator.map(_.range.asInstanceOf[Function]).toList.map(Dataset(_).toDoubleMap)
